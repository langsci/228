...
is transcribed phonemically and presented in dark blue italic font; e.g. \ve{fatu} `stone'.\footnote{
		There are only two non-phonemic aspects of my transcription.
		Firstly, /ɡw/ is transcribed \it{<g>} before rounded vowels (\srf{sec:VoiObs}).
...
is the verbal agreement prefix \ve{m-} `{\m}' which agrees with first person exclusive,
second person singular and second person plural.
Glosses follow the Leipzig Glossing Rules with exceptions listed on \prf{ch:Abb}.
...
\begin{exe}
	\ex{\glll	neno naa paha{\gap}ʔpina-n ia, a|n-kobub on bare meseʔ\\
						...
						day{\tbrU} {\naa} country{\gap}below{\tbrU}-{\N} {\ia} {\a\n}-pile.up{\tbrUc} {\on} place{\tbrU} one\\
...
When examples from recorded texts are cited,
