...
foot structure with with form CVVC.
The structures of the M-form of the words
in \qf{as:fafi}--\qf{as:nautus} above
are given in are given in \qf{as:faif}--\qf{as:naut} below.
...
\ve{kaut} `papaya', \ve{fafi} `pig' and \ve{ai} `fire',
	...
	\centering\caption{Ro{\Q}is Amarasi Post-stress /a/ Assimilation}\label{tab:RoqPosStrAAss}
		...
		\end{tabular}%}
			\begin{tablenotes}
				...
				\item [‡]	Kots Amarasi \ve{utan} means only `vegetables'.
...
a following empty C-slot. Thus, \ve{boʔ=es\tbr{a{\gap}} =m hiut} `seventeen'
and \ve{n-ok\tbr{a{\gap}} skoor} `with a school'.
...
A stem such as \ve{nima} {\ra} \ve{niim} `five'
...
These two rules and stipulation,
