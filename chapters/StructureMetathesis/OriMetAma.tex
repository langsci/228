...
are repeated in \trf{tab:RoqStrVSloDip},
...
More examples given in \trf{tab:SouAmaMetFor} on the next page.
...
%	\multirow{-2}{*}{\ve{{\rt}mnasi}}	&\multirow{-2}{*}{\ve{na-mnaasi̯}}	&[naˈmna̙ˑçː]
...
\qf{ex:NB-171026-4, 0.56} below which shows a
...
\begin{exe}
	...
	\ex{\glll	n-ak ``ena hoo mu-ho\tbr{in}'' n-ak ``au u-ho\tbr{oni̯}''\\
						...
						\n-ay \hp{``}mother {\hoo} \mu-born{\tbrM} \n-say \hp{``}{\au} \qu-born{\tbrM}\\
			\glt	`He said ``Mother, have you given birth?'', she said ``I've given birth''.' \txrf{NB-171026-4, 0.56}
...
Examples are given in \trf{tab:AmfConDel}.
...
discussed furhterin \srf{sec:CCIniMod} below.
