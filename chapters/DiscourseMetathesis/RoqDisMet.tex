...
connectors (\srf{sec:MfoDepCoo}) and the use of U-forms before negators (\srf{sec:RoqNeg}).
...
introduces a number of verbs that specify this concept further
and is resolved by the M-forms
of the parallel pair \ve{na-riikn na-peaʔn}.

\begin{exe}
	\ex{\glll	uis aafgw=ii, hiin naiʔ na-pe\tbr{ʔan}, na-moini-b, na-toro-b, na-ri\tbr{ikn}, nape\tbr{aʔn}.\\
...
For nouns to have such CC-final M-forms,
they must fulfil either set of criteria given in
...
\begin{exe}
	...
	\begin{xlist}
		\ex{Option 1:}\label{ex:ResRoqCCMforOpt1}
			\begin{xlist}
				\ex{Both the penultimate and final consonants are nasals}
			\end{xlist}
		\ex{Option 2:}\label{ex:ResRoqCCMforOpt2}
			\begin{xlist}
				\ex{The penultimate or final consonant is /n/}
...
while the other forms are used to mark discourse structures.
...
\begin{exe}
	\ex{\glll	\sf{tapi} karu n-mama =maeʔ, ahh n-me\tbr{up} =te, en na-peeh.\\
						...
						but if \n-chew ={\maeq} {} \n-work{\tbrM} ={\te} like \na-lazy\\
			\glt	`But if they don't chew betel nut, then when they work its like they're lazy'
...
However, when stem a with a final consonant other than
