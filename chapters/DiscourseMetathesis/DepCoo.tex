\section{Dependent co-ordination}\label{sec:DepCoo}
...
The typical structure of dependent co-ordination is given in \qf{ex:ChrCoo} below.
...
Each of the connectors \ve{=ma} and \ve{=te} have four allomorphs each.
...
\subsection{Dependent co-ordination with \it{=ma} `and'}\label{sec:Coo=Ma}
...
\begin{exe}
	\exp{ex:130928-1, 1.54}[*]{\glll	iin aam-f=ii esa n-re\tbr{un} =\tbr{ma} n-hain reʔ nopu\\
						...
						{\iin} father-{\F}={\ii} {\esc} \n-order{\tbrM} =\tbr{and} \n-dig{\M} {\reqt} hole\\
			\glt	`It was his\sub{\it{i}} father who gave the order and (he\sub{\it{i}}) dug the grave.' \txrf{el. 09/02/16 p.9}}
	\exp{ex:120715-4, 0.45 ch:DisMet}[*]{\glll	a|n-mo\tbr{eʔ} =\tbr{ma} n-poo\j=ena n-bi metoʔ\\
						{\a}n-moʔe =ma n-poi=ena n-bi metoʔ\\
						{\a\n}-make{\tbrM} =\tbr{and} \n-exit{\Mv}={\een} \n-{\bi} dry\\
			\glt	\lh{a|}`he made and (then) went out onto dry land.' \txrf{el. 13/02/16 p.16}}
\end{exe}
...
\begin{exe}
	\ex{\glll	fee mnaisʔ=ee na-su\tbr{un} =\tbr{ma} n-moeʔ on reʔ ia.\\
						fee mnasiʔ=ee na-suna =ma n-moʔe on reʔ ia\\
...
in which both verbs on either side of the connector \ve{=ma}
...
\begin{exe}
	\ex{\glll	mu-he\tbr{un} =\tbr{ma} mu-tiis paah{\gap}pina-n\\
						...
						{\mu}-fill{\tbrM} =\tbr{and} {\mut}-pour country{\gap}below-{\N}\\
...
One specific kind of dependent co-ordination with \ve{=ma} `and' involves large numbers.
...
\subsection{Dependent co-ordination with \it{=te} `\tsc{set}'}\label{sec:Coo=Te}
...
\begin{exe}
	\ex{\glll	nehh, \sf{{\j}adi} iin \bxA{n-ma\tbr{te}} =\tbr{te} \bxB{t-suub=ee} \hspace{25mm} on pani-n neefgw=ee? {\lk}\\
	...
	\ex{\glll	reko papa =m hoo \bxA{mu-hi\tbr{ni}} =\tbr{t} a|m-turan he \bxB{m-suir} m-aan =kau hee. {\lk}\\
						...
						good dad =and {\hoo} \gp\mu-know{\tbrU} {=\tbr{\te}} \a\m-help {\he} \gp\m-heal{\M} \m-\ana{\M} ={\kau} hey\\
...
\begin{exe}
		\ex{\glll	Mere, airoo, Mere, maans=ee \bxA{n-ma\tbr{be}} =\tbr{t} hoo \bxB{mu-kpesaʔ} {\lk}\\
							...
							Mary oh Mary sun={\ee} \gp\n-afternoon{\tbrU} =\tbr{\te} {\hoo} \gp\mu-sift{\Uc}\\
		...
		\ex{\glll	{nmeu{\gap}\bxA{n-fi\tbr{ni}}} =\tbr{t}, \bxB{n-aena} n-bi aat \sf{dees}=ii, =m n-ak {\lk}\\
				...
				\glt	`Early in the morning he ran up to the village (head)' \txrf{130825-6, 7.28} {\emb{130825-6-07-28.mp3}{\spk{}}{\apl}}}\label{ex:130825-6, 7.28}
\end{exe}
...
\begin{exe}
		...
		\ex{\glll	n-reuk \bxA{fa\tbr{nu}} =\tbr{te}, \sf{paʔ} \hp{Charl} Charles, \sf{paʔ} Graims a|n-koen=oo-n \bxB{neem.} {\lk}\\
							n-reku fanu =te \sf{paʔ} {} Charles \sf{paʔ} Graims {\a}n-koen=oo-n nema\\
							\n-hit eight{\tbrU} {=\tbr{\te}} Mr. {} Ch. Mr. Gr. \a\n-depart={\oo-\N} {\nema\M}\\
...
\begin{exe}
	\exp{ex:130902-1, 4.32 ch:DisMet}[*]{\glll
					...
					\glt	`In five days we'll elect a (new) regent.' \txrf{el. 22/02/16 p.21}}
	\exp{ex:130920-1, 0.51 ch:DisMet}[*]{\glll
								n-reuk fa\tbr{un} =\tbr{ate} paʔ Charles paʔ Graims a|n-koen=oo-n neem\\
								n-reku fanu =te paʔ Charles paʔ Graims {\a}n-koen=oo-n nema\\
								\n-hiit eight{\tbrM} {=\tbr{\te}} Mr. Ch. Mr. Gr. \a\n-depart={\oo-\N} {\nema}\\
					\glt	`As it struck 8:00 Mr. Charles, Mr. Grimes came.' \txrf{el. 13/02/16 p.15}}
\end{exe}
...
The inability for \ve{=te} to occur with an M-form is due to \ve{=te}
...
in dependent co-ordination is \ve{rari} `finish'.
...
\begin{exe}
	...
	\begin{xlist}
		\ex{\glll	{okeʔ =te}, hai m-ʔator, \sf{aʧara,} n-eu reʔ, ahh, \hp{oras toup tamu} oras toup \sf{tamu}, \sf{resepsi}\\
...
had finished (\ve{na-prir{\tl}riraʔ} `dances')
...
\begin{exe}
	\ex{Attending a wedding reception: \txrf{130902-1} {\emb{130902-1-03-23-03-34.mp3}{\spk{}}{\apl}}}\label{ex:130902-1, 3.23-3.28}
		\begin{xlist}
			...
			\ex{\glll	ahh, \tbr{rari} =\tbr{te}, n-ma-taeb n-ok ahh baroit=n=ein =ama hai m-tebi m-fain iim.\\
...
There are only three examples of of \ve{rari =te}
...
\begin{exe}
	...
	\ex{\glll	n-aka n-mani\tbr{ni} mes na-see\j=oo-n reʔ ia ro n-tuup=een.\\
...
This usage is not dissimilar from the
...
\begin{exe}
	\ex{The settling of Koro{\Q}oto hamlet: \txrf{160326} {\emb{160326-05-37-05-45.mp3}{\spk{}}{\apl}}}\label{ex:160326, 5.37-5.45}
		\begin{xlist}
		\ex{\glll	siin neem na-tua Koorʔoot ees reʔ oras mee \hspace{20mm} ka= ta-hiin t-a\tbr{na} =f.\\
...
\begin{exe}
	\ex{\glll	siin naiʔ Bain mone \sf{kusus}, siin kaan auk-k=e\tbr{ni} bisa, \hp{Mea aiʔ} Mea aiʔ Tutun.\\
						sini naiʔ Bani mone \sf{kusus} sini kana aku-k=eni bisa {} Mea aiʔ Tutun\\
						{\siin} {\naiq} Bani{\M} male exclusive {\siin} name praise.name-{\k=\ein\tbrU} can {} Mea or Tutun\\
			\glt	`Members of the Bani clan classified as male can
						exclusively have the praise names Mea or Tutun.'\footnote{
								In Amarasi society the classification of households as \ve{mone} `masculine'
								or \ve{feto} `feminine' refers to their social relationship to one another
...
\begin{exe}
	...
	\ex{\glll	\sf{paʔ} ʔnaak-- Inabuy ʔnaak aanʔ=ii n-bi Oekbi\tbr{it}.\\
...
U-forms of place names probably occur in other environments
in which discourse driven U-forms occur, such tail-head linkage (\srf{sec:TaiHeaLin})
and question-answer pairs (\srf{sec:IntUnm}).
However, in my current corpus I only have clear
examples in dependent co-ordination.
