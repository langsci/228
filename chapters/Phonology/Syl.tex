...
both which are discussed in (\srf{sec:TheFoo}).
...
\begin{exe}
	\exa{\xy
		...
		<-0.5em,0cm>*\as{[ˈ}="[",
		<0em,0cm>*\as{k}="kp",<0.7em,0cm>*\as{ə}="a1p",<1.3em,0cm>*\as{w}="up",
...
\begin{exe}
	...
	\sn{\gw\begin{tabular}{llll}
			...
			\ve{f\tbr{au}k}			&[f\tbr{əw}k]					&{\emb{fauk.mp3}{\spk{}}{\apl}}& `how many' \\
	\end{tabular}}
\end{exe}
...
Thus, in a particular word-list, the word \ve{hau} `tree, wood' occurs in isolation as
[ˈha.ʊ] {\emb{hau.mp3}{\spk{}}{\apl}},
without the second vowel being realised as an off-glide.
However, in the same word-list when the same word occurs in the compound \ve{hau noʔo} `tree leaf'
