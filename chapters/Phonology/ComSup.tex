...
The most unusual kind of consonant insertion I have so far encountered
...
root final /i/ is replaced by /ar/,\footnote{
		...
		= Amfo{\Q}an \ve{fafi\tbr{\j}} = Timaus \ve{fafa\tbr{r}} `pig'.
		Other instances of Amarasi /\j/ which correspond to Timaus /r/
		include Amarasi \ve{bi\tbr{\j}ae} Timaus \ve{bi\tbr{r}ael},
		and Amarasi \ve{nai\tbr{\j}eer} Timaus \ve{nai\tbr{r}eel} `ginger'.}
...
are given in \qf{ex:TimConIns2} below.
...
\begin{exe}
	...
	\sn{\gw\begin{tabular}{rlllll}
		...
		\ve{noʔo}&+&\ve{-n}&{\ra}& \ve{in noo-n}		& `its leaves'\\
...
To take just two examples, the word \ve{asu} `dog' with an empty initial
