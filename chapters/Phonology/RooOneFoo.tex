...
The proposed structures of \ve{kaunaʔ} {\ra} [ˈkəwnɐʔ]
	...
	\begin{exe}\ex{
		\begin{xlist}
			\exa{\xy
				...
				<-0.5em,0cm>*\as{[ˈ}="[",<0em,0cm>*\as{k}="kp",<0.7em,0cm>*\as{ə}="a1p",<1.3em,0cm>*\as{w}="up",
...
Firstly, as discussed in {\S}\ref{sec:Str} (\prf{ex:(C)VVCV(C)->"(C)VVCV(C)}),
...
\paragraph{Ro{\Q}is Amarasi diphthongisation}\label{sec:RoqAmaDip}
